                ...
                bool fCreateCert)
{
  ...
  sha1_get_info(2, &cbContext, &r_asnoid, &r_asnlen, &r_mdlen, &pfn_init, &pfn_write, &pfn_final, &pfn_read);
  ...
  char *pbHash = rgbSignature + sprintf(rgbSignature, "#1; bsign v%s\n", g_szVersion);
  ...
  if (fCreateCert)
  {
    ...
    if (rgbCert)
    {
      size_t cbCert = ((unsigned char *)rgbCert)[0] * 256 + ((unsigned char *)rgbCert)[1] + 2;
...
bool is_elf(char *pb, size_t cb)
{
  ...
  if (memcmp(header.rgbID, "\177ELF", 4) != 0 || header.bitclass < 1 || header.bitclass > 2 || header.byteorder < 1 || header.byteorder > 2 || _v(header.filetype) < 1 || _v(header.filetype) > 4 || (_v(header.ibHdrProgram) && _v(header.ibHdrProgram) < sizeof(header)) || _v(header.ibHdrProgram) >= cb || _v(header.ibHdrSection) < sizeof(header) || _v(header.ibHdrSection) >= cb || _v(header.cbHeader) != sizeof(header) || (_v(header.cbEntryProgram) && _v(header.cbEntryProgram) != sizeof(PROGRAM_ELF64)) || _v(header.cbEntrySection) != sizeof(SECTION_ELF64) || _v(header.iSectionNames) >= _v(header.cEntrySection))
    return false;

  // *** FIXME: I don't recall why we need more than a header test.

  const PROGRAM_ELF64 *rgProgram = (PROGRAM_ELF64 *)(pb + _v(header.ibHdrProgram));
  for (int i = 0; i < _v(header.cEntryProgram); ++i)
    if (_v(rgProgram[i].ib) >= cb || _v(rgProgram[i].ib) + _v(rgProgram[i].cbFile) > cb)
      return false;

  const SECTION_ELF64 *rgSection = (SECTION_ELF64 *)(pb + _v(header.ibHdrSection));
  for (int i = 1; i < _v(header.cEntrySection); ++i)
    if (_v(rgSection[i].sectiontype) != ELF_SECTION_NOBITS && (_v(rgSection[i].ib) >= cb || _v(rgSection[i].ib) + _v(rgSection[i].cb) > cb))
      return false;
                   ...
                   size_t *pibSignature, size_t *pcbSignature)
{
  ...
  const SECTION_ELF64 *rgSection = (SECTION_ELF64 *)(pb + _v(hdr.ibHdrSection));
...
eExitStatus check_elf(char *pb, size_t cb, bool fExpectSignature)
{
  ...
  size_t cbCert = (((unsigned char *)pb)[ibHash + 20] << 8) | ((unsigned char *)pb)[ibHash + 21];
...
eExitStatus hash_elf(char *pb, size_t cb, int fhNew, bool fSign)
{
  ...
  char *pbMap = (char *)mmap(NULL, cbRewrite, PROT_READ, MAP_FILE | MAP_PRIVATE, fhNew, 0);
  assert(pbMap);
                         ...
                         SECTION_ELF64 &sectionSig)
{
  ...
  unsigned64 cbSectionNamesNew = _v(sectionNames.cb) + sizeof(g_szSectionSig);
                        ...
                        const FIXUP64 *rgFixup)
{
  for (const FIXUP64 *pFixup = rgFixup; pFixup; pFixup = pFixup->pNext)
  {
    ...
    {
      cbOld = _v(unsigned64(_v(hdrOld.cbEntryProgram) * _v(hdrOld.cEntryProgram)));
    ...
    cbOld = _v(unsigned64(_v(hdrOld.cbEntrySection) * _v(hdrOld.cEntrySection)));
...
size_t rewrite_elf64(char *pb, size_t cb, int fh, FIXUP64 &fixupSignature)
{
  ...
  if (sizeof(hdrOld) != _v(hdrOld.cbHeader)) // We must have this
    return 0;
  if (_v(hdrOld.cEntryProgram) && sizeof(PROGRAM_ELF64) != _v(hdrOld.cbEntryProgram)) // We must have this
    return 0;
  if (_v(hdrOld.cEntrySection) && sizeof(SECTION_ELF64) != _v(hdrOld.cbEntrySection)) // We must have this
    return 0;

  const SECTION_ELF64 *rgSectionOld = (SECTION_ELF64 *)(pb + _v(hdrOld.ibHdrSection));
  ...
  {
    if (_v(rgSectionOld[i].sectiontype) == unsigned32(ELF_SECTION_SIGNATURE))
    {
  ...
  const PROGRAM_ELF64 *rgProgramOld = (PROGRAM_ELF64 *)(pb + _v(hdr.ibHdrProgram));
  PROGRAM_ELF64 *rgProgram = hdr.ibHdrProgram ? new PROGRAM_ELF64[_v(hdr.ibHdrProgram)] : NULL;
  if (rgProgram)
    memcpy(rgProgram, rgProgramOld,
           _v(hdr.cEntryProgram) * _v(hdr.cbEntryProgram));
  SECTION_ELF64 *rgSection = new SECTION_ELF64[_v(hdr.cEntrySection) + (iSectionOldSignature
                                                                            ? 0
                                                                            : 1)];
  ...
  char *rgbSectionNames = new_sectionnames64(pb, cb, sectionNames, sectionSig);
  ...
  while (ib < cbNew)
  {
    ...
    for (pFixup = pFixupHead; pFixup; pFixup = pFixup->pNext)
      if (pFixup->ibNew >= ib && (pFixupNext == NULL || pFixup->ibNew < pFixupNext->ibNew))
        pFixupNext = pFixup;
